...
DEFAULT_SWEEP_JSON = PILOT_DIR / "load_sweep_results.json"
...
def main() -> None:
    ...
    sweep_payload = json.loads(args.sweep_json.read_text(encoding="utf-8"))
    ...
    sweep_by_load = {float(item["q_mpa"]): item for item in sweep_results}
    missing = [float(load) for load in loads if float(load) not in sweep_by_load]
    if missing:
        raise RuntimeError(f"Sweep JSON is missing loads present in cache: {missing}")
    ...
    save_json(args.output_json, output_payload)
    ...
    for idx, load in enumerate(loads):
        ...
        output_payload["per_load_results"] = per_load_results
        output_payload["status"] = f"completed_{float_key(float(load))}"
        save_json(args.output_json, output_payload)

    overall_bulk_mismatch = np.array([sweep_by_load[float(load)]["overall"]["mean_bulk_rel_l2"] for load in loads], dtype=float)
    variable_bulk_mismatch = {
        name: np.array([sweep_by_load[float(load)]["variables"][name]["bulk"]["rel_l2"] for load in loads], dtype=float)
    ...
    for key in GROUP_CATALOG:
        ...
        high_idx = int(np.argmax(bulk_series))
        group_summary[key] = {
            "family": family,
            ...
            "peak_bulk_rel_l2_load_mpa": float(loads[high_idx]),
    ...
    earliest_groups = sorted(
        GROUP_CATALOG,
        key=lambda key: (group_summary[key]["first_significant_load_mpa"] is None, group_summary[key]["first_significant_load_mpa"] or float("inf"), -group_summary[key]["high_load_bulk_rel_l2"]),
    )
    largest_high_load_groups = sorted(
        GROUP_CATALOG,
        key=lambda key: group_summary[key]["high_load_bulk_rel_l2"],
        reverse=True,
    )
    strongest_overall_correlates = sorted(
        GROUP_CATALOG,
    ...
    theta0p_rank = sorted(theta0p_subgroups, key=lambda key: group_summary[key]["high_load_bulk_rel_l2"], reverse=True)

    summary = {
        "significance_threshold_bulk_rel_l2": SIGNIFICANCE_THRESHOLD,
        "overall_bulk_mismatch_growth_ratio": float(overall_bulk_mismatch[-1] / max(overall_bulk_mismatch[0], 1.0e-12)),
        "first_significant_loads_mpa": {
            key: group_summary[key]["first_significant_load_mpa"]
            for key in GROUP_CATALOG
        },
        "earliest_significant_groups": earliest_groups[:5],
        "largest_high_load_groups": largest_high_load_groups[:5],
        "strongest_overall_correlates": strongest_overall_correlates[:5],
        "theta0p_high_load_component_rank": list(theta0p_rank),
        "interpretation": {
            "theta0": (
            ...
            "Phi0p": (
                ...
                "tracking but not a proof that the shallow model omits the same effect."
            ),
        },
    }

    output_payload["group_summary"] = group_summary
    output_payload["summary"] = summary
    output_payload["status"] = "completed"
    save_json(args.output_json, output_payload)

    print("=== Pilot 13 term attribution ===")
    print(f"Loads analyzed: {', '.join(float_key(load) for load in loads)} MPa")
    print(f"Earliest significant groups: {', '.join(summary['earliest_significant_groups'])}")
    print(f"Largest groups at highest load: {', '.join(summary['largest_high_load_groups'])}")
